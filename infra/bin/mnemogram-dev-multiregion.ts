...
// Multi-region deployment for development/testing
...
regions.forEach(region => {
  ...
  new MnemogramStack(app, `MnemogramStack-dev-${regionSuffix}`, {
    stage: "dev",
    env: {
      account: process.env.CDK_DEFAULT_ACCOUNT,
      region: region,
    },
    description: `Mnemogram — Serverless AI Memory Service (DEVELOPMENT - ${region})`,
  });

  // API-only stack for regional endpoints  
  new MnemogramApiStack(app, `MnemogramApiStack-dev-${regionSuffix}`, {
    stage: "dev",
    env: {
      account: process.env.CDK_DEFAULT_ACCOUNT,
      region: region,
    },
    description: `Mnemogram API Stack (DEVELOPMENT - ${region})`,
  });
});
