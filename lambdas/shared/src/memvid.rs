...
impl MemvidClient {
    ...
    async fn download_mv2_file(&self, memory_id: &str) -> Result<NamedTempFile, MnemogramError> {
        ...
        let data: bytes::Bytes = obj.body.collect().await
    ...
    pub async fn search(&self, memory_id: &str, query: &str, top_k: usize) -> Result<Vec<MemvidSearchResult>, MnemogramError> {
        ...
        let output = AsyncCommand::new(memvid_path)
            ...
            .output()
            .await
            .map_err(|e| MnemogramError::ExternalService(format!("Failed to execute memvid find: {}", e)))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(MnemogramError::ExternalService(format!("memvid find failed: {}", stderr)));
        }
    ...
    pub async fn ask(&self, memory_id: &str, question: &str, top_k: usize) -> Result<MemvidAskResult, MnemogramError> {
        ...
        let output = AsyncCommand::new(memvid_path)
            ...
            .output()
            .await
            .map_err(|e| MnemogramError::ExternalService(format!("Failed to execute memvid ask: {}", e)))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(MnemogramError::ExternalService(format!("memvid ask failed: {}", stderr)));
        }
            ...
            if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(line) {
                let result = MemvidSearchResult {
                    ...
                    timestamp: json_value.get("timestamp")
                        ...
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string()),
                    frame_id: json_value.get("frame_id")
                        .or_else(|| json_value.get("id"))
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string()),
                    uri: json_value.get("uri")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string()),
                };
    ...
    pub async fn validate_mv2_file(&self, memory_id: &str) -> Result<bool, MnemogramError> {
        ...
        let obj = self.s3_client
            ...
            .send()
            .await
            .map_err(|e| MnemogramError::S3Error(format!("Failed to read .mv2 file header: {}", e)))?;

        let data: bytes::Bytes = obj.body.collect().await
            .map_err(|e| MnemogramError::S3Error(format!("Failed to read .mv2 file header data: {}", e)))?
            .into_bytes();
...
pub fn get_memvid_version() -> Result<String, MnemogramError> {
    ...
    if output.status.success() {
        ...
        Err(MnemogramError::ExternalService("Failed to get memvid version".to_string()))
    }
}
