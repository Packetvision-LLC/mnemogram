...
impl Validator {
    ...
    pub fn required_string(value: Option<&str>, field_name: &str) -> Result<String, MnemogramError> {
        match value {
            Some(v) if !v.trim().is_empty() => Ok(v.trim().to_string()),
            _ => Err(MnemogramError::ValidationError(
        ...
        if let Some(min_len) = min {
            if len < min_len {
                return Err(MnemogramError::ValidationError(
                    format!("Field '{}' must be at least {} characters long", field_name, min_len)
                ));
            }
        }
    ...
    pub fn positive_integer(value: i64, field_name: &str) -> Result<(), MnemogramError> {
        if value <= 0 {
            return Err(MnemogramError::ValidationError(
                format!("Field '{}' must be a positive integer", field_name)
            ));
        }
        Ok(())
    }
    ...
    pub fn number_range(
        ...
        if let Some(max_val) = max {
            if value > max_val {
                return Err(MnemogramError::ValidationError(
                    format!("Field '{}' cannot exceed {}", field_name, max_val)
                ));
            }
        }
    ...
    pub fn file_size(size_bytes: u64, field_name: &str, max_mb: u64) -> Result<(), MnemogramError> {
        ...
        if size_bytes > max_bytes {
            return Err(MnemogramError::ValidationError(
                format!("File '{}' cannot exceed {}MB (current size: {}MB)", 
                    field_name, 
                    max_mb, 
    ...
    pub fn file_type(filename: &str, allowed_extensions: &[&str]) -> Result<(), MnemogramError> {
        ...
        if !allowed_extensions.iter().any(|&ext| ext == extension) {
            return Err(MnemogramError::ValidationError(
                format!("File type '{}' not allowed. Allowed types: {}", 
                    extension, 
    ...
    pub fn memory_name(name: &str) -> Result<(), MnemogramError> {
        ...
        if !name.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_' || c == ' ') {
            return Err(MnemogramError::ValidationError(
                "Memory name can only contain letters, numbers, spaces, hyphens, and underscores".to_string()
            ));
        }
    ...
    pub fn pagination(limit: Option<i64>, offset: Option<i64>) -> Result<(i64, i64), MnemogramError> {
